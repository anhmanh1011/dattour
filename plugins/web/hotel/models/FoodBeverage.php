<?php namespace Web\Hotel\Models;

use Model;
use BackendAuth;

/**
 * Model
 */
class FoodBeverage extends Model
{
    use \October\Rain\Database\Traits\Validation;
    
    use \October\Rain\Database\Traits\SoftDelete;

    protected $dates = ['deleted_at'];

    /**
     * @var array Validation rules
     */
    public $rules = [
    ];


    //Đa ngôn ngữ
    public $implement = ['@RainLab.Translate.Behaviors.TranslatableModel'];
    public $translatable = [
        'name',
        'description',
        'content',
        ['slug', 'index' => true]
    ];

    /**
     * @var string The database table used by the model.
     */
    public $table = 'web_hotel_food_beverage';

    public $belongsTo = [
        'location' =>[
            'Web\Hotel\Models\Location',
            'table' => 'web_hotel_relation_hotel_location',
            'order' => 'name'
        ],
        'kind' =>[
            'Web\Hotel\Models\KindofFoodBeverage',
            'table' => 'web_hotel_kind_food_beverage',
            'order' => 'name'
        ],
        'type' =>[
            'Web\Hotel\Models\TypeofFoodBeverage',
            'table' => 'web_hotel_type_food_beverage',
            'order' => 'name'
        ]
    ];

    
    public $attachOne = [
        'image_preview' => ['System\Models\File', 'order' => 'sort_order']
        
    ];
    public $attachMany = [
        'album_images' => ['System\Models\File', 'order' => 'sort_order']
        
    ];
    public function filterFields($fields, $context = null)
    {
        $user = BackendAuth::getUser();

        if (!$user->hasAnyAccess(['web.hotel.access_delete'])) {
            $fields->active->hidden = true;
            $fields->hot->hidden = true;
        }
        else {
            $fields->active->hidden = false;
            $fields->hot->hidden = false;
        }
    }
    public function scopeListFrontEnd($query, $options = []){
        extract(array_merge([
            'page' => 1,
            'perPage' => 36,
            'sort' => 'created_at desc',
            'state' => null,
            'type' => '',
            'trang' => 1,
        ], $options));
        if($trang!=1){dd($trang);}
        if($state !== null) {
            if(!is_array($state)){
                $state = [$state];
            }
            $query->whereHas('location', function($q) use ($state){
                $q->whereIn('states_id', $state);
            });
        }
        if($type != null) {
            if(!is_array($type)){
                $type = [$type];
            }
            // foreach ($type as $loai){
            //     $query->whereHas('type', function($q) use ($loai){
            //         $q->where('id', '=', $loai);
            //     });
            // }
            $query->whereHas('type', function($q) use ($type){
                $q->whereIn('id', $type);
            });
        }
        $lastPage = $query->paginate($perPage, $page)->lastPage();
        if($lastPage < $page){
            $page = 1;
        }
        return $query->where('active', '=', 1)->orderBy('created_at', 'asc')->paginate($perPage, $page);
    }
}
